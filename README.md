...
2.4 Create a bucket with python on and push data in bucket
First install boto3 in IDE of Python “pip install boto3” 
Import boto3 and with the help of resources or clients create a new s3 bucket, the name of the bucket is unique.
Push the file or data in a bucket with key upload_file.

2.5 Read a data from Bucket
Import boto3 and  create a client and make a path 
First s3://
Second bucketname/
Next File name
...
3.1 Spark connect with s3 and read data
 Boto.s3.connection function for connection of s3 server to local,
 We pass access key and secret key,
 Read data from s3 bucket with path  # getting error while using spark to read data, read data from pandas and save .csv file, 
Clean data because it has special characters is object so convert into float with udf function.

3.2 Train linear model
Help of Spark ML vectorization to convert data inputcols, outputcol. Input as low, high, open and output close.
Import Linearregression model due to prediction of continuous values
Fit the model with splitting 70 and 30 data.
Train the model and test model
In testing find accuracy and prediction values.
...
Install Zookeeper from Apache server,
Install Kafka from Apache server.
...
5.0 Create a Alpha vantage  and read stock data of different companies
Open alpha vantage web page provide a gmail and details then you get key,
Install alpha vantage in pip3, 
Provide key, company name and time limit of data. Producer.
Create Produce data.
...
Install flask,
Import Flask 
Create Templates and index.html
Develop Flask and Display in a web page read graphical data. 
